              ...
              <AnimateOnScroll key={product.id} delay={index * 100}>
                <div className="bg-white rounded-2xl overflow-hidden shadow-sm h-full flex flex-col transition-transform duration-300 hover:scale-105 hover:shadow-lg">
                  <div className="relative aspect-[4/3] overflow-hidden">
                    <Image src={product.image_urls?.[0] || product.image_url?.split(',')[0] || '/images/placeholder.jpg'} alt={product.name} fill className="object-cover" />
                  </div>
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-xl font-bold text-primary mb-2">{product.name}</h3>
                    <div className="flex-1"></div>
                    <div className="flex items-end justify-between">
                      {product.price_note && <p className="text-base font-bold leading-relaxed whitespace-pre-line" style={{ color: '#AA7452' }}>{product.price_note.replace(/\s*[/／]\s*/g, '\n')}</p>}
                      <Link href={`/products/detail/${product.id}`} className="squish-btn px-4 py-2 text-white text-sm font-medium rounded-lg inline-block flex-shrink-0 ml-3" style={{ backgroundColor: '#AA7452' }}>查看詳情</Link>
                    </div>
                  </div>
                </div>
              </AnimateOnScroll>
