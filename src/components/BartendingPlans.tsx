  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#fdfcfb] text-[#172039]">
      ...
      <section className="relative mx-auto max-w-[1400px] border-b border-[#e2ded8] px-5 py-16 md:px-10 md:py-20 lg:px-16">
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-5">{serviceFeatures.map((item, index) => <div key={`${item}-${index}`} className="text-center md:border-r md:border-[#dedbd5] md:px-5 md:last:border-r-0"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-[#cda774] text-[#b58445]"><Check size={20} strokeWidth={1.4} /></div><h3 className="mt-4 text-base font-medium text-[#172039]">{item.split('：')[0]}</h3><p className="mt-2 text-sm leading-6 text-[#5b5e65]">{item.includes('：') ? item.split('：').slice(1).join('：') : '依活動需求提供現場服務與規劃。'}</p></div>)}</div>
