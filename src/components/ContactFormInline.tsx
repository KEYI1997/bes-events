...
export default function ContactFormInline() {
  const [form, setForm] = useState({
    name: '', phone: '', email: '', service_type: '', event_date: '', budget: '', description: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true);
        setForm({ name: '', phone: '', email: '', service_type: '', event_date: '', budget: '', description: '' });
      } else {
        setError('提交失敗，請稍後再試');
      }
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-primary mb-1">活動日期</label>
          <input
            type="date"
        ...
        <div>
          <label className="block text-sm font-medium text-primary mb-1">預算範圍</label>
          <input
            type="text"
            value={form.budget}
            onChange={(e) => setForm({...form, budget: e.target.value})}
            className="w-full px-4 py-3 border border-primary/20 rounded-lg focus:ring-2 focus:ring-cta focus:border-transparent bg-white"
            placeholder="例：5萬-10萬"
          />
        </div>
      </div>
      <div>
