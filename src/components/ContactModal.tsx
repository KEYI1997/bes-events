...
export default function ContactModal({ isOpen, onClose, productName, productId, serviceType, addOnOptions = [], choiceOptions = [], initialExtraSelection, onExtraSelectionChange, priceOptions = [], initialPriceOptionValue = '', orderQuantity }: ContactModalProps) {
  ...
  const hasLockedPriceOption = lockedPriceOptions.length > 0;
  const selectedRegularPriceOption = priceOptions.find((option, index) => optionKey(option, index) === selectedPriceOption || `${option.label}｜${option.price}` === selectedPriceOption);
  const selectedPriceOptions = hasLockedPriceOption
