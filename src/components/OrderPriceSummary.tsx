...
import { formatProductAddOnPrice, formatProductAmount, formatProductPrice, optionKey, productOptionTotals, productPriceAmount, selectedAddOnQuantity, type ProductOptionRow } from '@/lib/productOptions';

type OrderPriceSummaryProps = {
  productItems: ProductOptionRow[];
  addOnItems?: ProductOptionRow[];
  addOnItemKeys?: string[];
...
function SummaryRows({ items, emptyMessage, addOn = false, addOnQuantities = {}, addOnItemKeys = [] }: { items: ProductOptionRow[]; emptyMessage: string; addOn?: boolean; addOnQuantities?: Record<string, number>; addOnItemKeys?: string[] }) {
  if (!items.length) return <p className="rounded-lg bg-white/70 px-3 py-2 text-sm text-[#7b746c]">{emptyMessage}</p>;

  return <div className="space-y-2">
    {items.map((item, index) => {
      const addOnQuantity = selectedAddOnQuantity({ addOnQuantities }, addOnItemKeys[index] || optionKey(item, index));
      const addOnAmount = productPriceAmount(item.price.replace(/^\+\s*/, ''));
      const addOnSubtotal = addOn && addOnAmount !== null ? addOnAmount * addOnQuantity : null;
      return <div key={`${item.id || item.label}-${index}`} className="flex items-start justify-between gap-3 rounded-lg bg-white/70 px-3 py-2">
        <span className="min-w-0 text-sm font-medium text-[#4A4947]">{item.label}</span>
        <span className="shrink-0 text-right text-sm font-semibold text-[#8f5d3f]">
          <span className="block">{addOn ? formatProductAddOnPrice(item.price) : formatProductPrice(item.price) || '洽詢'}{addOn && <span className="ml-1 tabular-nums text-[#6f665f]">× {addOnQuantity.toLocaleString('zh-TW')}</span>}</span>
          {addOnSubtotal !== null && <span className="mt-0.5 block text-xs font-medium text-[#6f665f]">小計 {formatProductAmount(addOnSubtotal)}</span>}
        </span>
      </div>;
    })}
  </div>;
...
export default function OrderPriceSummary({ productItems, addOnItems = [], addOnItemKeys = [], addOnQuantities = {}, quantity = 1, total, emptyTotalMessage = '請先選擇商品規格' }: OrderPriceSummaryProps) {
  ...
  return (
    <aside aria-live="polite" aria-label="價格總覽" className="rounded-2xl bg-[#F8F2EC] p-5 text-[#4A4947] shadow-[0_12px_30px_rgba(89,63,43,0.12)]">
      ...
      {addOnItems.length > 0 && <section className="mt-5">
        ...
        <SummaryRows items={addOnItems} emptyMessage="尚未選擇加購商品" addOn addOnQuantities={addOnQuantities} addOnItemKeys={addOnItemKeys} />
      </section>}
