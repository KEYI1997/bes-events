...
export default function ProductExtrasSelection({ addOns, choices, selection, onChange, basePrice = '', quantity = 1 }: {
  ...
  const choiceGroups = choices.reduce<Record<string, ChoiceEntry[]>>((groups, row, index) => {
    const groupName = row.group?.trim() || '選配商品';
  ...
  return <div className="space-y-6">
    ...
    {Object.entries(choiceGroups).map(([groupName, entries], groupIndex) => <fieldset key={groupName}>
      <legend className="mb-3 font-semibold text-[#4A4947]">{groupName}<span className="ml-2 text-sm font-normal">不加價・限選一項</span></legend>
      <div className="grid gap-3 sm:grid-cols-2">
