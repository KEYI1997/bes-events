...
export async function buildQuotationPdf(order: QuotationPdfData): Promise<Buffer> {
  ...
  const noteTop = y + 8;
  doc.roundedRect(MARGIN, noteTop, CONTENT_WIDTH, 48, 3).lineWidth(0.6).strokeColor(BORDER).stroke();
  doc.fillColor(BRAND).fontSize(9).text('付款與報價說明', MARGIN + 8, noteTop + 6);
  ...
  doc.fillColor(INK).fontSize(6.4).text(noteLines.join('\n'), MARGIN + 8, noteTop + 19, { width: CONTENT_WIDTH - 16, lineGap: 0, height: 27, ellipsis: true });

  const sectionY = noteTop + 59;
  doc.moveTo(MARGIN, sectionY - 5).lineTo(PAGE_WIDTH - MARGIN, sectionY - 5).lineWidth(0.8).strokeColor(BRAND).stroke();
  doc.fillColor(INK).fontSize(12).text('回簽前請詳閱說明', MARGIN, sectionY, { width: CONTENT_WIDTH, align: 'center' });
  doc.fillColor(MUTED).fontSize(6.8).text('本頁為報價單之一部分，簽署或付款即表示同意以下約定。', MARGIN, sectionY + 18, { width: CONTENT_WIDTH, align: 'center' });
  ...
  let termY = sectionY + 32;
  terms.forEach((term, i) => { doc.fillColor(BRAND).fontSize(6.4).text(`${i + 1}.`, MARGIN, termY, { width: 17, align: 'right' }); doc.fillColor(INK).fontSize(6.4).text(term, MARGIN + 22, termY, { width: CONTENT_WIDTH - 22, lineGap: 0 }); termY += 14; });

  const signatureTop = termY + 6; const signatureGap = 12; const signatureWidth = (CONTENT_WIDTH - signatureGap) / 2;
  doc.roundedRect(MARGIN, signatureTop, signatureWidth, 58, 3).lineWidth(0.6).strokeColor(BORDER).stroke();
  doc.roundedRect(MARGIN + signatureWidth + signatureGap, signatureTop, signatureWidth, 58, 3).lineWidth(0.6).strokeColor(BORDER).stroke();
  doc.fillColor(BRAND).fontSize(8).text('廠商簽章', MARGIN + 8, signatureTop + 7, { width: signatureWidth - 16, align: 'center' });
  doc.fillColor(BRAND).fontSize(8).text('客戶簽章', MARGIN + signatureWidth + signatureGap + 8, signatureTop + 7, { width: signatureWidth - 16, align: 'center' });
  if (fs.existsSync(stampPath)) doc.image(stampPath, MARGIN + 45, signatureTop + 17, { fit: [66, 38], align: 'center', valign: 'center' });
  const customerSignX = MARGIN + signatureWidth + signatureGap + 18;
  doc.moveTo(customerSignX, signatureTop + 42).lineTo(customerSignX + signatureWidth - 36, signatureTop + 42).lineWidth(0.5).strokeColor(BORDER).stroke();
  doc.fillColor(MUTED).fontSize(6.2).text('簽名／蓋章', customerSignX, signatureTop + 46, { width: signatureWidth - 36, align: 'center' });
  const bankTop = signatureTop + 65;
  doc.roundedRect(MARGIN, bankTop, CONTENT_WIDTH, 30, 3).fill(SOFT);
  doc.fillColor(BRAND).fontSize(7).text('匯款資訊', MARGIN + 8, bankTop + 7, { width: 56 });
  doc.fillColor(INK).fontSize(6.4).text('戶名：境曜有限公司｜國泰世華銀行（013）古亭分行｜帳號：030035016755', MARGIN + 66, bankTop + 6, { width: CONTENT_WIDTH - 76 });
  doc.fillColor(MUTED).fontSize(5.8).text('匯款後請提供帳號末五碼，方便工作人員核對。', MARGIN + 66, bankTop + 18, { width: CONTENT_WIDTH - 76 });
  const footerY = bankTop + 40;
